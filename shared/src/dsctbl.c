...
void init_gdtidt(void){
  ...
  GATE_DESC* idt = (GATE_DESC*)ADDR_IDT;
  int i;
  for(i = 0; i < LIMIT_GDT / 8; ++i){
    set_segmdesc(gdt + i, 0, 0, 0);
  }
  set_segmdesc(gdt + 1, 0xffffffff, 0x00000000, AR_DATA32_RW);
  set_segmdesc(gdt + 2, LIMIT_BOOTPAK, ADDR_BOOTPAK, AR_CODE32_ER);
  ...
  // IDTの設定
  set_gatedesc(idt + 0x0c, (int)asm_interrupt_handler0c, 2*8, AR_INTGATE32);
  set_gatedesc(idt + 0x0d, (int)asm_interrupt_handler0d, 2*8, AR_INTGATE32);
